...
exports.keystoneAuth = function(req, res, next) {
	var requireHttps = (req.headers['x-forwarded-proto'] !== 'https' && keystone.get('secure admin')),
		prefix = requireHttps ? keystone.get('secure admin') : '',
		from = new RegExp('^\/keystone\/?$', 'i').test(req.url) ? '' : '?from=' + req.url;

	if (!req.user || !req.user.canAccessKeystone || requireHttps) {
		return res.redirect(prefix + keystone.get('signin url') + from);
	}
